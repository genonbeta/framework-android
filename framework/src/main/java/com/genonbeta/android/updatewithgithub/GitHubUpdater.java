...
// TODO: 19.02.2020 Use new file saving methods
        ...
        new Thread()
        {
            ...
            public void run()
            {
                ...
                try {
                    ...
                    final PackageInfo packInfo = mContext.getPackageManager().getPackageInfo(
                    ...
                    if (releases.length() > 0) {
                        ...
                        for (int iterator = 0; iterator < releases.length(); iterator++) {
                            ...
                            if (listener != null)
                                listener.onInfoAvailable(comLast.compareTo(comCurr) > 0, lastVersionName,
                                        lastVersionTitle, lastVersionBody, lastVersionDate);

                            final File updateFile = new File(Environment.getExternalStoragePublicDirectory(
                                    Environment.DIRECTORY_DOWNLOADS) + "/" + applicationName + " v" + lastVersionName + ".apk");
                                    ...
                                    if (releaseAssets.length() > 0) {
                                        ...
                                        if (updateFile.isFile()) {
                                            ...
                                            dialog.setMessage(String.format(mContext.getString(
                                                    R.string.genfw_uwg_update_body), appVersionName, lastVersionName,
                                                    lastVersionDate, lastVersionBody))
